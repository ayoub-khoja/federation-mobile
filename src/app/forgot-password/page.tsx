...
export default function ForgotPasswordPage() {
  ...
  return (
    <div className={`h-screen max-h-screen flex items-start justify-center pt-16 relative overflow-hidden ${isRtl ? 'rtl' : 'ltr'}`} dir={isRtl ? 'rtl' : 'ltr'} style={{overflow: 'hidden'}}>
      ...
      {/* Container principal */}
      <div className="relative z-10 w-full max-w-7xl px-3 sm:px-6 flex items-center justify-center">
        
        {/* Section centrale - Formulaire de récupération */}
        <div className="flex-1 max-w-sm sm:max-w-lg">
          {/* Carte de récupération */}
          <div className="glass rounded-xl sm:rounded-2xl shadow-ftf overflow-hidden animate-fadeInUp">
            
            {/* Header avec logos */}
            <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4 text-center">
              ...
              <h1 className={`text-base sm:text-lg font-bold text-red-600 mb-2 ${isRtl ? 'font-arabic' : ''}`}>
                {forgotPasswordT.title}
              </h1>
              <p className={`text-gray-600 text-xs sm:text-sm ${isRtl ? 'font-arabic' : ''}`}>
            ...
            <div className="px-4 sm:px-6 pb-3 sm:pb-4">
              {!isEmailSent ? (
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="bg-gradient-to-r from-red-600 to-red-700 rounded-lg p-2 sm:p-3 text-center">
                    <div className="flex items-center justify-center mb-1">
                      <img 
                        src="/Logo_federation_tunisienne_de_football.svg.png" 
                        alt="Logo FTF" 
                        className="h-5 sm:h-6 w-auto mr-1 sm:mr-2"
                      />
                    </div>
                  ...
                  <div className="text-center px-2">
                    <h3 className={`text-xs sm:text-sm font-semibold text-gray-800 mb-0.5 ${isRtl ? 'font-arabic' : ''}`}>
                      {language === 'fr' ? `Bonjour ${email.split('@')[0]},` : `مرحباً ${email.split('@')[0]}،`}
                    </h3>
                    <p className={`text-gray-600 text-xs leading-tight ${isRtl ? 'font-arabic' : ''}`}>
                      {language === 'fr' 
                  ...
                  <div className="bg-amber-50 border-l-4 border-amber-400 p-1.5 sm:p-2 rounded-r-lg mx-2 sm:mx-0">
                    <div className="flex items-center mb-1">
                      <svg className="w-3 h-3 text-amber-600 mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                      </svg>
                      <span className={`text-amber-800 font-medium text-xs ${isRtl ? 'font-arabic' : ''}`}>
                        {language === 'fr' ? 'Code OTP de sécurité :' : 'رمز OTP الأمني:'}
                      </span>
                    </div>
                    
                    {/* Code OTP stylisé */}
                    <div className="bg-white border-2 border-red-300 rounded-lg p-1.5 sm:p-2 text-center">
                      <div className="text-sm sm:text-base font-bold text-red-700 tracking-widest">
                  ...
                  {/* Instructions */}
                  <div className="bg-gray-50 rounded-lg p-1.5 sm:p-2 mx-2 sm:mx-0">
                    <h4 className={`font-semibold text-gray-800 mb-1 text-xs ${isRtl ? 'font-arabic' : ''}`}>
                      {language === 'fr' 
                        ? 'Pour réinitialiser votre mot de passe, vous devez :'
                        : 'لإعادة تعيين كلمة المرور، يجب عليك:'
                      }
                    </h4>
                    <ol className={`space-y-0.5 sm:space-y-1 text-xs text-gray-600 ${isRtl ? 'font-arabic' : ''}`}>
                      <li className="flex items-start">
                        <span className="bg-red-600 text-white text-xs rounded-full w-3 h-3 flex items-center justify-center mr-1 mt-0.5 flex-shrink-0">1</span>
                      ...
                      <li className="flex items-start">
                        <span className="bg-red-600 text-white text-xs rounded-full w-3 h-3 flex items-center justify-center mr-1 mt-0.5 flex-shrink-0">2</span>
                        <span className="leading-tight">
                          {language === 'fr' 
                      ...
                      <li className="flex items-start">
                        <span className="bg-red-600 text-white text-xs rounded-full w-3 h-3 flex items-center justify-center mr-1 mt-0.5 flex-shrink-0">3</span>
                        <span className="leading-tight">
                          {language === 'fr' 
                  ...
                  <div className="space-y-1.5 sm:space-y-2 px-2 sm:px-0">
                    {timeLeft > 0 ? (
                      <div className="text-center bg-red-50 rounded-lg p-1.5 sm:p-2">
                        <p className={`text-xs text-gray-700 ${isRtl ? 'font-arabic' : ''}`}>
                      ...
                      <div className="text-center space-y-1.5">
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-1.5">
                          <p className={`text-xs text-amber-800 ${isRtl ? 'font-arabic' : ''}`}>
                        ...
                        <button
                          ...
                          disabled={isLoading('forgotPassword')}
                          className={`w-full bg-blue-600 text-white py-1.5 sm:py-2 rounded-lg font-semibold hover:bg-blue-700 transition-colors shadow-md hover:shadow-lg transform hover:scale-105 active:scale-95 duration-200 disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none text-xs ${isRtl ? 'font-arabic' : ''}`}
                        >
                          {isLoading('forgotPassword') ? (
                  ...
                  <div className="px-2 sm:px-0">
                    <button
                      onClick={handleBackToLogin}
                      className={`w-full bg-red-600 text-white py-1.5 sm:py-2 rounded-lg font-semibold hover:bg-red-700 transition-colors shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95 duration-200 text-xs ${isRtl ? 'font-arabic' : ''}`}
                    >
                  ...
                  <div className="bg-blue-50 border-l-4 border-blue-400 p-1.5 rounded-r-lg mx-2 sm:mx-0">
                    <div className="flex items-start">
                      ...
                      <p className={`text-blue-800 text-xs leading-tight ${isRtl ? 'font-arabic' : ''}`}>
                        {language === 'fr' 
