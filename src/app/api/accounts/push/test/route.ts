...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🧪 Test des notifications push - Connexion au backend:', backendUrl);
    
    try {
      const response = await fetch(`${backendUrl}/accounts/push/test/`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { error: errorData.detail || 'Erreur lors du test des notifications' },
